...
drivers <- list(
  Ta     = c("Temperature"),
  Precip = c("precip1", "precip2"),
  CO2    = c("CO2_global"),
  LWDR   = c("LWdown")
  # extend until all 80 features are covered
)

# Step 2. Helper: detrend one variable relative to reference year
detrend_to_ref <- function(df, var, id_col="ID", year_col="Year", ref_year=1960) {
  df %>%
    group_by(!!sym(id_col)) %>%
    mutate(
      ref_val = mean(.data[[var]][.data[[year_col]] == ref_year], na.rm=TRUE),
      model = list(lm(.data[[var]] ~ .data[[year_col]])),
      pred = predict(model[[1]], newdata=data.frame(Year=.data[[year_col]])),
      !!var := .data[[var]] - (pred - ref_val)
    ) %>%
    ungroup() %>%
    select(-ref_val, -model, -pred)
}
...
make_scenario <- function(df, driver_vars, type="detrend", ref_year=1960) {
  ...
  for (v in driver_vars) {
    if (type == "detrend") {
      df_new <- detrend_to_ref(df_new, v, id_col="ID", year_col="Year", ref_year=ref_year)
    } else if (type == "fix") {
...
scenarios <- list()
scenarios[["Base"]] <- poc_data_clean
for (drv in names(drivers)) {
  scenarios[[paste0("NoTrend_", drv)]] <- make_scenario(
    poc_data_clean, drivers[[drv]], type="detrend", ref_year=1960
  )
}
...
for (sc in names(scenarios)) {
  df <- scenarios[[sc]]
  X_new <- df %>% select(all_of(colnames(X))) %>% mutate_all(as.numeric)
...
for (drv in names(drivers)) {
  ...
  contrib <- base_trend %>%
    left_join(control_trend, by="ID", suffix=c("_Base","_Control")) %>%
