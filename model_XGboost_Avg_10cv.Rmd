...
# basin POC flux
library(lubridate)
# Calculate Cpoc uncertainty: SD of model prediction error (Predicted - Observed)
if ("Cpoc" %in% names(new_outlet_data) & "Predicted_Cpoc" %in% names(new_outlet_data)) {
  cpoc_sd <- sd(new_outlet_data$Predicted_Cpoc - new_outlet_data$Cpoc, na.rm = TRUE)
} else {
  cpoc_sd <- NA_real_
}
...
basin_result <- new_outlet_data %>%
  ...
  summarise(
    ...
    Cpoc_SD = cpoc_sd,                               # Cpoc uncertainty (global SD)
...
basin_monthly <- basin_result %>%
  mutate(
    ...
    # SE propagation for product: F = C * Q * k
    se_Fpoc_basin_month = abs(Fpoc_basin_month) * sqrt(
      (sd(Avg_Cpoc, na.rm = TRUE) / pmax(mean(Avg_Cpoc, na.rm = TRUE), .Machine$double.eps))^2 +
      (sd(Sum_Q,    na.rm = TRUE) / pmax(mean(Sum_Q,    na.rm = TRUE), .Machine$double.eps))^2
    )
  )
...
basin_annual <- basin_monthly %>%
  ...
  summarise(
    ...
    se_Fpoc_basin_year = sqrt(sum(se_Fpoc_basin_month^2, na.rm = TRUE)),
    .groups = "drop"
  )
...
national_annual <- basin_annual %>%
  ...
  summarise(
    ...
    se_Fpoc_all = sqrt(sum(se_Fpoc_basin_year^2, na.rm = TRUE)),
    .groups = "drop"
  )
...
ggplot(national_annual, aes(x = Year, y = Fpoc_all)) +
  ...
  geom_ribbon(aes(ymin = Fpoc_all - se_Fpoc_all,
                  ymax = Fpoc_all + se_Fpoc_all),
              fill = "red", alpha = 0.2) +
  labs(title = "Time series of national POC flux",
       subtitle = paste0("Trend slope: ", round(coef(lm(Fpoc_all ~ Year, data = national_annual))[2], 4), " Tg/year/year"),
       x = "Year",
       y = "Fpoc (Tg/year)") +
...
annual_data <- basin_monthly %>%
  ...
  summarise(
    ...
    se_Cpoc   = sd(Avg_Cpoc, na.rm = TRUE) / sqrt(n()),
    mean_Q    = mean(Sum_Q, na.rm = TRUE),
    se_Q      = sd(Sum_Q, na.rm = TRUE) / sqrt(n()),
    sum_Fpoc  = sum(Fpoc_basin_month, na.rm = TRUE),  # Tg/year
    se_Fpoc   = sd(Fpoc_basin_month, na.rm = TRUE) / sqrt(n()),
    .groups = "drop"
  )
...
annual_all <- annual_data %>%
  ...
  summarise(
    ...
    se_Cpoc   = sd(mean_Cpoc, na.rm = TRUE) / sqrt(n()),
    mean_Q    = mean(mean_Q, na.rm = TRUE),
    se_Q      = sd(mean_Q, na.rm = TRUE) / sqrt(n()),
    sum_Fpoc  = sum(sum_Fpoc, na.rm = TRUE),
    se_Fpoc   = sqrt(sum(se_Fpoc^2, na.rm = TRUE)),
    .groups = "drop"
  ) %>%
...
annual_long <- annual_data %>%
  pivot_longer(
    cols = c(mean_Cpoc, mean_Q, sum_Fpoc, se_Cpoc, se_Q, se_Fpoc),
    names_to = c(".value", "Metric"),
    names_pattern = "(mean|sum|se)_(.*)"
  ) %>%
  group_by(BasinName, Metric) %>%
  mutate(
    norm_mean = mean / max(mean, na.rm = TRUE),
    norm_se   = se   / max(mean, na.rm = TRUE)
  ) %>%
  ungroup()
...
make_plot <- function(basin_name, show_legend = FALSE) {
  ...
  p <- ggplot(df, aes(x = Year, y = norm_mean, color = Metric, fill = Metric)) +
    geom_line(size = 1) +
    geom_point(size = 2) +
    geom_smooth(method = "lm", se = FALSE, linetype = "dashed", color = "black") +
    scale_color_manual(values = c("Q"="purple","Cpoc"="darkgreen","Fpoc"="red")) +
    scale_fill_manual(values = c("Q"="purple","Cpoc"="darkgreen","Fpoc"="red")) +
    labs(title = basin_name, x = NULL, y = NULL,
         subtitle = paste0("Trend slope: ",
           paste(sapply(unique(df$Metric), function(m) {
             slope <- coef(lm(norm_mean ~ Year, data = df[df$Metric == m,]))[2]
...
```{r Check Monthly Avg}
annual_data %>% filter(is.na(se_Cpoc) | is.na(se_Q))
summary(annual_data)
```
