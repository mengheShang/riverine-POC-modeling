...
print(st_crs(china_map))

# 定义投影
proj <- st_crs(china_map)

# 将shapefile转换到指定投影坐标系
china_map_proj <- st_transform(china_map, crs = proj)
nineline_proj <- st_transform(nineline, crs = proj)
basin_proj <- st_transform(basin, crs = proj)
stem_river_proj <- st_transform(stem_river, crs = proj)
tri_river_proj <- st_transform(tri_river, crs = proj)

# 将散点数据转换为sf对象并转换到指定投影坐标系
data_sf <- st_as_sf(data, coords = c("Longitude", "Latitude"))
data_sf_proj <- st_transform(data_sf, crs = proj)

# 创建共用地图基础
base_map <- ggplot() +
  geom_sf(data = china_map_proj, fill = "ivory", color = "black", alpha = 1) +
  geom_sf(data = nineline_proj, fill = "ivory", color = "black") +
  geom_sf(data = basin_proj, fill = NA, color = "black") +
  geom_sf(data = tri_river_proj, fill = NA, color = "lightblue", linewidth = 0.4) +
  geom_sf(data = stem_river_proj, fill = NA, color = "lightblue", linewidth = 0.7) +
  geom_sf(data = data_sf_proj, aes(size = Sampling_Count_Breaks, color = Sampling_Time_Breaks), 
          shape = 21, lwd = 10) +
